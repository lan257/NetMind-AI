...
defineEmits(['created', 'update:viewMode', 'preview-node']);
</script>
...
<template>
  <section class="create-page">
    <div class="create-grid">
      ...
      <section class="panel create-card">
        ...
        <div class="field-row ai-actions">
          ...
          <el-button :icon="MagicStick" data-testid="ai-clean" :loading="workspace.loading.value" @click="workspace.cleanNaturalLanguage">
            自然语言清洗
          </el-button>
          <el-button :icon="ChatDotRound" data-testid="ai-open-chat" @click="workspace.chatOpen.value = true">
            需求对话
          </el-button>
        </div>
      ...
      <section class="panel create-card">
        ...
        <div class="transfer-actions">
          <el-button :icon="Download" data-testid="export-structure" :disabled="!workspace.selectedMap.value" @click="workspace.exportStructure">
            导出结构
          </el-button>
          <el-button :icon="Download" data-testid="export-file" :disabled="!workspace.selectedMap.value" @click="workspace.downloadSelectedMap">
            导出文件
          </el-button>
          <el-button data-testid="download-template" @click="workspace.downloadUrl('/api/mind-map-transfer/template')">
            模板
          </el-button>
          <el-button :icon="Upload" data-testid="import-structure" @click="workspace.importStructure">
            导入结构
          </el-button>
        </div>
        <div class="field-row transfer-import-row">
          ...
          <input
            :ref="(el) => { workspace.fileInput.value = el; }"
            data-testid="import-file"
            type="file"
            accept="application/json,.json"
            @change="workspace.importFile"
          />
        </div>
        <el-input
          v-model="workspace.transferText.value"
    ...
    >
      ...
      <el-input
        v-model="workspace.chatInput.value"
